...
describe("controle positivo — o produto sem marca não pode se mexer", () => {
  it("a Sage reproduz, pintada, os números do design system", () => {
    ...
    // Claro: os números medidos contra as superfícies slate (slate-50 e slate-100).
    expect(foco(p.claro, "--color-bg")).toBeCloseTo(3.82, 2);
    expect(foco(p.claro, "--color-surface-elevated")).toBeCloseTo(3.65, 2);
    // Escuro: medido contra superfícies slate escuras (slate-900 e slate-700).
    expect(foco(p.escuro, "--color-bg")).toBeCloseTo(6.15, 2);
    expect(foco(p.escuro, "--color-surface-elevated")).toBeCloseTo(3.57, 2);
    // No escuro o anel contra -soft composto em superfícies slate.
    expect(foco(p.escuro, "--color-accent-soft@--color-surface")).toBeCloseTo(4.2, 2);
    expect(foco(p.escuro, "--color-accent-soft@--color-surface-elevated")).toBeCloseTo(3.05, 2);
  });
...
describe("a navy #0f172a — o defeito que a prova em tela achou", () => {
  it("os quatro números do anel de foco, agora acima do piso", () => {
    ...
    expect(foco(p.claro, "--color-bg")).toBeCloseTo(10.83, 2);
    expect(foco(p.claro, "--color-surface-elevated")).toBeCloseTo(10.35, 2);
    expect(foco(p.escuro, "--color-bg")).toBeCloseTo(9.27, 2);
    expect(foco(p.escuro, "--color-surface-elevated")).toBeCloseTo(5.38, 2);
